...
import React, { useMemo, useState, useRef, useCallback, forwardRef, ForwardedRef } from 'react';
...
const createNodeLabels = (
  ...
  sampleNodes.forEach(node => {
    ...
    if (needsLine || connectionDistance > baseNodeSize * 3.0) {
      ...
      const lineOpacity = Math.min(255, Math.max(140, 255 - connectionDistance * 6));
...
const SpatialArg3DVisualization = React.forwardRef<HTMLDivElement, SpatialArg3DProps>(({
  ...
  }, [nodes3D, nodeIdSettings, combinedNodesForTransform, combinedEdgesForTransform, nodeSizes, colors, customLabelPositions]);
  ...
  const layers = [
    ...
    new ScatterplotLayer<Node3D>({
      ...
      getFillColor: (d: Node3D) => calculateNodeColor(d, selectedNode || null, temporalRange || null, temporalFilterMode, colors),
      getLineColor: (d: Node3D) => calculateNodeOutlineColor(d, selectedNode || null, data, temporalRange || null, temporalFilterMode, colors),
      ...
      updateTriggers: {
        getRadius: [nodeSizes],
        getLineWidth: [nodeSizes]
      },
      onClick: (info: any, event: any) => {
        // Left click only - right click will be handled separately
