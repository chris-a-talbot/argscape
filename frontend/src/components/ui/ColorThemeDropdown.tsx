...
const colorThemeOptions: ColorThemeOption[] = [
  ...
  // Temporarily disabled: { value: 'custom', label: 'Custom', icon: '🎭' },
];
...
export const ColorThemeDropdown: React.FC = () => {
  ...
  const getDisplayLabel = () => {
    return currentOption?.label || 'Light';
  };

  const getDisplayIcon = () => {
    return currentOption?.icon || '☀️';
  };
  ...
  return (
    <>
      <div className="relative" ref={dropdownRef}>
        ...
        {isOpen && (
          <div
            className="absolute right-0 mt-1 py-1 rounded-md shadow-lg min-w-[160px]"
            style={{
              backgroundColor: colors.containerBackground,
              border: `1px solid ${colors.border}`,
              zIndex: 10000
