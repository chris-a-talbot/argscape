...
export default function SpatialDiffTreeSequenceSelector({ onSelect, className = '' }: SpatialDiffTreeSequenceSelectorProps) {
      ...
      <div className={className}>
        <div className="bg-sp-dark-blue border border-sp-pale-green/20 rounded-xl p-8 text-center">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h3 className="text-lg font-semibold text-sp-white">No Tree Sequence Loaded</h3>
          </div>
          <p className="text-sp-white/60 text-sm">Please load a tree sequence first before comparing.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`flex flex-col space-y-4 ${className}`}>
      ...
      <div className="bg-sp-pale-green/10 border-2 border-sp-pale-green/30 rounded-xl p-5">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-8 h-8 bg-sp-pale-green/20 rounded-lg flex items-center justify-center">
            <svg className="w-4 h-4 text-sp-pale-green" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </div>
          <h3 className="text-lg font-semibold text-sp-pale-green">Currently Loaded Tree Sequence</h3>
        </div>
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <span className="font-mono text-sp-white text-sm font-medium truncate" title={currentTreeSequence.filename}>
              {currentTreeSequence.filename}
            </span>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs text-sp-white/80">
            <div>
              <span className="text-sp-white/60">Samples:</span>
              <span className="font-mono font-bold text-sp-pale-green ml-2">{currentTreeSequence.num_samples.toLocaleString()}</span>
            </div>
            <div>
              <span className="text-sp-white/60">Nodes:</span>
              <span className="font-mono font-bold text-sp-pale-green ml-2">{currentTreeSequence.num_nodes.toLocaleString()}</span>
            </div>
            <div>
              <span className="text-sp-white/60">Edges:</span>
              <span className="font-mono font-bold text-sp-pale-green ml-2">{currentTreeSequence.num_edges.toLocaleString()}</span>
            </div>
            <div>
              <span className="text-sp-white/60">Trees:</span>
              <span className="font-mono font-bold text-sp-pale-green ml-2">{currentTreeSequence.num_trees.toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-sp-dark-blue border border-sp-pale-green/20 rounded-xl p-5 flex-1 flex flex-col">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex-1">
            <h3 className="text-lg font-semibold text-sp-white">Select Tree Sequence to Compare</h3>
            <p className="text-sm text-sp-white/60 mt-1">
        ...
        {matchingSequences.length === 0 ? (
          <div className="flex-1 flex items-center justify-center py-12">
            <div className="text-center max-w-md">
              ...
              <h4 className="text-lg font-semibold text-sp-white mb-2">No Matching Tree Sequences Found</h4>
              <p className="text-sm text-sp-white/70 mb-4">
                No other tree sequences were found with identical structure to the currently loaded sequence.
              </p>
              <div className="bg-sp-very-dark-blue border border-sp-pale-green/20 rounded-lg p-4 text-left space-y-2">
                <p className="text-xs text-sp-white/80 font-semibold">To compare tree sequences, they must have:</p>
                <ul className="text-xs text-sp-white/60 space-y-1 list-disc list-inside">
                ...
                <p className="text-xs text-sp-white/60 mt-3">
              ...
              <button
                ...
                className="mt-4 bg-sp-dark-blue hover:bg-sp-pale-green hover:text-sp-very-dark-blue text-sp-white border border-sp-pale-green/20 font-bold py-2 px-4 rounded-lg transition-all duration-200 flex items-center gap-2 mx-auto"
              >
          ...
          <>
            ...
            {/* Action Button */}
            <div className="mt-4 pt-4 border-t border-sp-pale-green/20">
              <button
                onClick={handleSelect}
                disabled={!selectedFilename || !treeSequenceInfos[selectedFilename]}
                className="w-full bg-sp-pale-green hover:bg-sp-very-pale-green text-sp-very-dark-blue font-bold py-3 px-6 rounded-xl transition-all duration-200 transform hover:scale-105 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none flex items-center justify-center gap-2"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
                </svg>
